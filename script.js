...
ScrollReveal().reveal('.home-image, .services-container, .Portfolio-box, .portfolio-slider, .contact form', { origin: 'top' });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.addEventListener("resize", () => {
    document.querySelectorAll(".portfolio-slider").forEach((slider) => {
      ...
      const images = slider.querySelectorAll(".slides img");
      const counter = 0;
      slides.style.transform = `translateX(${-images[0].clientWidth * counter}px)`;
    });
  });
});
